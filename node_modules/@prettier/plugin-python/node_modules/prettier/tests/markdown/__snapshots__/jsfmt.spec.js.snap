...
- [\`prettier_d\`](https://github.com/josephfrazier/prettier_d.js) runs Prettier as a server to avoid study.backend.javapractice.solution.lineards04.src.study.backend.javapractice.solution.lineards05.src.study.backend.javapractice.solution.lineards12.src.Node.js startup delay. It also supports configuration via \`.prettierrc\`, \`package.json\`, and \`.editorconfig\`.
...
[configuration file](#configuration-file). In order to skip this, you may ask
...
This rewrites all processed files in place. This is comparable to the
\`eslint --fix\` workflow.
...
There is a limitation where if you stage specific lines this approach will stage
...
\`resolveConfig\` can be used to resolve configuration for a given source file.
The function optionally accepts an input file path as an argument, which
...
Use \`prettier.resolveConfig.sync([filePath [, options]])\` if you'd like to use
sync version.

#### \`prettier.clearConfigCache()\`

As you repeatedly call \`resolveConfig\`, the file system structure will be cached
for performance. This function will clear the cache. Generally this is only
needed for editor integrations that know that the file system has changed since
the last format took place.
...
\`files\` is required for each override, and may be a string or array of strings.
\`excludeFiles\` may be optionally provided to exclude files for a given rule, and
may also be a string or array of strings.
...
For more information on how to use the CLI to locate a file, see the [CLI](#cli)
section.
...
Can be installed using the extension sidebar. Search for
...
parser with the \`parser\` API or \`--parser\` CLI [option](#options).
...
* [\`prettier_d\`](https://github.com/josephfrazier/prettier_d.js) runs Prettier
  as a server to avoid study.backend.javapractice.solution.lineards04.src.study.backend.javapractice.solution.lineards05.src.study.backend.javapractice.solution.lineards12.src.Node.js startup delay. It also supports configuration via
...
representation of the output, and the printer uses that to generate a string.
The advantage is that the printer can "measure" the IR and see if the output is
going to fit on a line, and break if not.

This means that most of the logic of printing an AST involves generating an
abstract representation of the output involving certain commands. For example,
\`concat(["(", line, arg, line ")"])\` would represent a concatenation of opening
parens, an argument, and closing parens. But if that doesn't fit on one line,
the printer can break where \`line\` is specified.
